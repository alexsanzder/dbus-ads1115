...
class TankSensor:
    ...
    def __init__(self, config, dbus=None):
        ...
        # Attach to settings
        self._settings_base = {
            'scale': [f'/Settings/Devices/device0_{self._id}/Scale', 1.0, 0.0, 1.0],
            'offset': [f'/Settings/Devices/device0_{self._id}/Offset', 0, 0, ADS1115_RANGE]
        }
    ...
    def _attach_to_dbus(self, dbus):
        ...
        _dbus = VeDbusService(f"{TankSensor.dbusBasepath}device0_{self._id}", bus=dbus)
    ...
    def update(self):
        ...
        try:
            ...
            self._dbus["/Level"] = round(self._level, 1)
            self._dbus["/Remaining"] = round(self._remaining, 4)
